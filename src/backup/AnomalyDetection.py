...
class AnomalyDetection:
    ...
    def adaptive_image_to_binary(self,img):
        ...
        # blurring to reduce noise, very important before adaptive thresholding
        blurred_image = cv2.GaussianBlur(gray_image, tuple(detect_lcd_params.kernel_blur_size), 0)

        binary_image = cv2.adaptiveThreshold(
            blurred_image,
            maxValue=255,
            adaptiveMethod=cv2.ADAPTIVE_THRESH_MEAN_C,  # ADAPTIVE_THRESH_MEAN_C,ADAPTIVE_THRESH_GAUSSIAN_C  
            thresholdType=cv2.THRESH_BINARY,
            blockSize=detect_lcd_params.block_size,   # the size of the surrounding area, must be odd number
            C=detect_lcd_params.C    # the value to subtract after averaging, used to adjust sensitivity
        )
    ...
    def get_top_left_point_box(self,box):
        ...
        # calculate the sum x+y, the top left point will have the smallest sum
        s = box.sum(axis=1)
    ...
    def detect_ccd(self, img):
        """Detect the CCD area and return the relevant results"""
        self.is_detected_CCD = False
    ...
    def train(self):
        """Function for training the anomaly detection model"""
